# 世界杯预测模型 V4.5 (回测 75.5%·路径倒挂·第三名概率·赛制完善)

> 📅 2026-06-27 · 球员DB 1250人/48队 · 8因子Logit链·动态弃权·context生死战·Flow=0.8
> 🆕 V4.5: 路径倒挂检测·第三名晋级概率·淘汰赛路径期望难度·公平竞赛占位·66场数据
...
| **V4.3** | 06-26 | 🆕 中场DB-Static量纲统一·穿盘率专项回测·波胆格局化输出·伪警告消除·54场73.5% |
| **V4.4** | 06-27 | 🆕 平局独立升级(三触发)·状态分赛事权重·灰区标记·高信平局二次校验·回测75.5% |
| **V4.5** | 06-27 | 🆕 路径倒挂检测(L2地狱·L3更优)·第三名晋级概率·淘汰赛期望难度·公平竞赛占位·66场 |

## V4.5 赛制完善 + 路径倒挂 (06-27)

### 第三名晋级概率 (`knockout_motivation.py`)

`_estimate_third_place_prob()`: 静态快照·12组第三名排序·概率分级 (85%/65%/35%/15%/5%)。
非线性映射: >60%=高·30-60%=观望·<30%=低·替代二值判断。

### 淘汰赛路径期望难度

`_estimate_third_path_difficulty()`: 12个头名难度中位数(~2.0)·替代兜底值9。
`_get_knockout_opponent(pos=3)` 返回真实期望对手。

### 路径倒挂检测 (`_is_path_inverted`)

检测低排名反而有更优淘汰赛路径:
- L组: L1=2 / L2=9 / L3=2 → L2是地狱·输球(L3)比平局(L2)更好
- J组: L1=2 / L2=10 / L3=2 → 同样倒挂
- 触发条件: l2 > l1+4 AND l2 > l3+4 (双重·仅极端)

`determine_scenario_enhanced` 修正:
- pos=3 + draw_enough → base 4.0 (平局=完美结果·赢了反入地狱)
- pos=3 + must_win → base 8.0 (先确保出线)

`calculate_motivation` 追加:
- 站在最差路径(L2) → avoidance +1.5
- 站在最优路径 → avoidance +0.5

### 公平竞赛占位 (P3)

`_get_standings_sorted` 排序元组追加 `h2h_pts, h2h_gd, fair_play` (第4-6键·占位)。

### 数据更新

66场比赛 (新增今日12场: G/H/I/A/B/C 组完赛)。

## V4.4 平局升级 + 状态分权重 (06-27)

### 平局独立升级 (`pre_match_report.py` `_build_structured()`)

三触发条件 (零误杀·仅升级高确定性平局):
- **触发A**: 泊松平局≥30% + 平赔暴跌critical → 上限55%
- **触发B**: 泊松平局>主胜+8pp 且 >客胜+8pp → 上限60%
- **触发C**: 双方平局即可出线 → 上限50%

回测: 捕获1场 (巴拉圭VS澳大利亚0-0)·零误杀·MOD 88%

### 高信平局二次校验

置信度≥70%时检测平局风险信号 (critical→-10, high→-5), 防止高信平局漏判。

### 灰色区间标记

40-50%置信度自动标记"灰区·平局风险", 该区间历史准确率仅44%。

### 状态分赛事权重 (`recent_form.py`)

`weigh_by_recency()` 新增赛事类型加权:
- 淘汰赛 1.5× / 小组赛 1.2× / 预选赛 1.0× / 友谊赛 0.6×

权重常量移到 `config.py` (待回测校准)。

### 比分格局化输出 (`score_prediction.py`)

"最可能比分"→"比分格局", 按主胜/平局/客胜分组, 波胆可靠性声明(Top-1仅15%)。
...
### 穿盘率专项回测 (`backtest_cover_rate.py`)

54场全量回测穿盘率与置信度关系:
- 穿盘率低档(20-30%): 11/13=85% 最佳
- 穿盘率高档(50-60%): 3/6=50% 最差
- 穿盘率与准确率呈反向关系
- 金三角约束: 4/4=100% 全部正确

### 波胆回测

47场有效: Top-1=14.9% / Top-3=36.2% / Top-8=59.6%
- 1-1 出现在 94% 场次的 Top-3 (泊松低分区密度堆积·非平局预测)
- 19场未命中全部总进球≥4 (泊松λ上限不足)
- 结论: 精确波胆不可靠·格局参考有价值

### 比分输出格局化 (`score_prediction.py`)

`format_score_output()` 重构:
- "最可能比分"→"比分格局" (主胜/客胜·大胜/小胜格局)
- 按结果类型分组 (主胜 X% · 平局 Y%)
- 波胆可靠性声明 (Top-1仅15%·仅供参考)
- 大胜格局不出虚假具体比分
- 单点<15%追加"看格局不押单点"提示
...
- [[v44-draw-upgrade]] — V4.4平局升级·状态分赛事权重·回测75.5%
