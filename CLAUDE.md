# 世界杯预测模型 V3.34 (回测 28/31=90.3%)

> 📅 2026-06-22 · 球员DB 1250人/48队 · 决策树11路径 · 六轮审计修复 · 三条件FW收紧 · 穿盘风险告警
...
## V3.34 修复 (06-22, 四轮审计驱动)

| # | 问题 | 文件 | 修复 |
|:--:|------|------|------|
| F1 | 三条件(b)五大联赛FW≥5M仍可通过 | `opponent_db.py` | has_top5_fw→直接❌ |
| F2 | 报告中"真过热"与dimension12覆盖混淆 | `pre_match_report.py` | 标签改为"是→dimension12覆盖·假过热" |
| F3 | 穿盘率区间标签不显示实际值 | `score_prediction.py` | 标签含实际值如`低穿盘(30%∈[20,30))` |
| F4 | 亚盘与竞彩让球未区分 | `pre_match_report.py` | 标注`基于竞彩让球盘口·与亚盘属不同维度` |
| F5 | 精英例外摘要不可见 | `pre_match_report.py` | `MOD+真过热→默认不胜\|⚠️FIFA覆盖→热门胜` |
| F6 | 胜率>60%+穿盘<45%无风险告警 | `pre_match_report.py` | `⚠️赢球输盘风险`告警+原始穿盘检查 |
| BF | 共识急剧翻转(>50pp)误奖励 | `pre_match_report.py` | 翻转→×0.92惩罚 |
| BF | BIG真过热惩罚不足 | `pre_match_report.py` | ×0.85→×0.70 |
| BF | 穿盘-泊松矛盾惩罚偏弱 | `pre_match_report.py` | -5→-10·下限30→25 |
| BF | 伊朗场外自动加成误判 | `pre_match_report.py` | 移除自动+8%保留警告 |

## 数据接入

- **竞彩让球赔率**: 从让球指数XLS自动提取竞*官*行→报告XLS区`🎫竞彩让球`
- **竞彩总进球**: `totals_odds_config.json` 手动配置→报告展示+投注建议调用
- **赔率来源**: 胜平负=百家欧赔·让球=竞彩·总进球=手动·比分=估算

## 预测快照

- `predictions_june22_23.txt` — 8场完整报告 (V3.34·2026-06-22)
