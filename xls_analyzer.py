    ...
    for i, line in enumerate(lines):
        ...
        if i == 4:  # Row 5: 最小值
            data["min"] = {"win": cells[1], "draw": cells[2], "lose": cells[3],
                           "win_prob": cells[4], "draw_prob": cells[5], "lose_prob": cells[6]}
    # 修复: 配对行结构(奇数行=初盘, 偶数行=即时)
    companies = []
    j = 6
    while j < len(lines) - 1:
        ca = lines[j].split('|||')
        nm = ca[0].strip() if len(ca) > 0 else ""
        if nm and nm not in ['最大值','最小值','即时','初盘','离散值','平均值']:
            iw = ca[1] if len(ca)>1 else ""; idr = ca[2] if len(ca)>2 else ""; il = ca[3] if len(ca)>3 else ""
            cb = lines[j+1].split('|||')
            nw = cb[1] if len(cb)>1 else ""; ndr = cb[2] if len(cb)>2 else ""; nl = cb[3] if len(cb)>3 else ""
            companies.append({"name":nm,"init_win":iw,"init_draw":idr,"init_lose":il,"now_win":nw,"now_draw":ndr,"now_lose":nl})
            j += 2
        else:
            j += 1
    data["companies"] = companies[:20]

    # P1-1: 自动计算平赔信号
    draw_signal = {"init_draw": None, "now_draw": None, "direction": "unknown", "change_pct": 0}
    for c in companies:
        if c['name'] and '均值' in c['name']:
            try:
                di = float(c['init_draw']); dn = float(c['now_draw'])
                draw_signal = {"init_draw": di, "now_draw": dn,
                               "direction": "DOWN" if dn < di else "UP" if dn > di else "FLAT",
                               "change_pct": round((dn - di) / di * 100, 2)}
            except: pass
            break
    data["draw_signal"] = draw_signal
    return data
